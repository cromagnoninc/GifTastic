var giphy = ["Booger", "Poop", "Heaven's Gate", "Charles Manson", "Wonder Showzen","Goatsee" ];

// Creates buttons when the page loads 
creategiphyButtons();


// When the button with the ID of add is clicked, a new variable is saved as the value that the user entered
// with the excess spaces trimmed off. This value is then pushed to the end of the animals array and the createAnimalButtons
...
$('#addgiphy').on('click', function() {
    var giphyEntered = $('#giphyInput').val().trim();
    giphy.push(giphyEntered);
    $('#giphyInput').val('');
    creategiphyButtons();
...
$(document.body).on('click', '.button-list', function() {
    ...
    var giphyClicked = $(this).data('giphy');
    // Creates a variable to hold the query string for the Giphy API request and adds the aniamls name to the query string.
    var query = 'https://api.giphy.com/v1/gifs/search?q=' + giphyClicked + '&limit=10&api_key=dc6zaTOxFJmzC';
...
function creategiphyButtons() {
    ...
    for (var i = 0; i < giphy.length; i++) {
        ...
        button.attr('data-giphy', giphy[i]).html(giphy[i]);
